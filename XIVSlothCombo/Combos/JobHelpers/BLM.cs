...
{
    internal class BLMOpenerStateMachine : PvE.BLM
    {
        enum State
        {
            None,
        ...
        public bool advance(ref uint actionID)
        {
            ...
            if (state == State.None)
            {
                ...
                if (canAdvance)
                {
                    state = State.FirstSharpcast;
                }
            }
            if (state == State.FirstSharpcast)
            {
                bool hasCorrectState = true;
                hasCorrectState &= CustomComboFunctions.LocalPlayer.CurrentMp == 10000;
                hasCorrectState &= CustomComboFunctions.GetRemainingCharges(Triplecast) == 2;
                hasCorrectState &= CustomComboFunctions.IsOffCooldown(Manafont);
                hasCorrectState &= CustomComboFunctions.IsOffCooldown(All.Swiftcast);
                hasCorrectState &= CustomComboFunctions.HasCharges(Sharpcast);
                if (!hasCorrectState)
                { 
                    state = State.None;
                    return false;
            ...
            if (state == State.Fire3)
            {
                ...
                hasCorrectState &= CustomComboFunctions.IsOffCooldown(All.Swiftcast);
                hasCorrectState &= CustomComboFunctions.HasCharges(Sharpcast);
                hasCorrectState &= CustomComboFunctions.WasLastAction(Sharpcast);
            ...
            if (state == State.FirstThunder3)
            {
                bool hasCorrectState = true;
                hasCorrectState &= CustomComboFunctions.LocalPlayer.CurrentMp >= 8000;
                hasCorrectState &= CustomComboFunctions.GetRemainingCharges(Triplecast) == 2;
                hasCorrectState &= CustomComboFunctions.IsOffCooldown(Manafont);
                hasCorrectState &= CustomComboFunctions.IsOffCooldown(All.Swiftcast);
                hasCorrectState &= CustomComboFunctions.HasCharges(Sharpcast);
                hasCorrectState &= CustomComboFunctions.WasLastAction(Fire3);
                if (!hasCorrectState)
                {
                    state = State.None;
                    return false;
